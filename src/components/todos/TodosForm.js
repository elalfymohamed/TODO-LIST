...
const TodosForm = ({addTodoHandler, showUncompleteHandle, mode, todos}) => {
    ...
    return (
        <div className={'todos-form'}>
            <div className={mode === 'not-done' ?  'todos-form_icon active': 'todos-form_icon'} onClick={showUncompleteHandle}>
                <FeatherIcon icon='circle'/>
            </div>
            <div className={'todos-form_form'}>
                <input type='text' placeholder='اضف مهمة جديدة ....' onChange={newTodoHandler} value={newTitle}/>
            </div>
            <div className={'todos-form_submit'}>
                <button type='button' className={mode === 'edit' ? "btn edit-btn" : 'btn'} onClick={addNewTodoHandler}
